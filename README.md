...
| `/큐모드` | 큐 모드 ON/OFF 전환 |
| `/스킵` | 현재 재생 중인 TTS 스킵 |
| `/전체스킵` | 현재 큐에 남아있는 TTS 전체 스킵 |
| `/청소` | 조건에 맞는 채팅 내역 삭제 (범위/기간/작성자/내용 선택) |
...
<details>
<summary><strong>2026-08-14</strong> — 음성 연결 끊김 대응 개선, 목소리 속도 설정 반영, 전체 스킵 명령어 추가</summary>

* 마우스로 음성 연결을 강제로 끊었을 때의 동작이 개선되었습니다.
  * `/입장`으로 다시 들어오면 끊긴 동안 쌓인 채팅을 한꺼번에 몰아서 읽는 문제가 재입장 시 대기열이 초기화되어 그 이후 채팅부터만 읽습니다.
  * `/입장` 없이도 다음 채팅이 오면 봇이 자동으로 다시 들어가도록 개선했습니다.
* 웹페이지에서 설정한 목소리 속도가 실제 재생에는 반영되지 않던 문제를 수정했습니다.
  * 이제 설정한 속도대로 모든 목소리(edge, 로컬, 영어)에서 정상적으로 재생됩니다.
* 대기 중인 문장을 한번에 모두 건너뛸 수 있는 `/전체스킵` 명령어가 추가되었습니다.
  * 현재 재생 중인 문장과 큐에 쌓인 문장을 전부 지우고 싶을 때 사용할 수 있습니다.

</details>

<details>
<summary><strong>2026-08-03</strong> — TTS 재생 안정성 개선, 숫자 읽기 개선, 에러 로그 필터 추가</summary>

* 채팅이 몰릴 때 TTS 재생 순서가 뒤바뀌던 문제를 수정했고, 실제 환경에서 안정성을 계속 확인하고 있습니다.
  * 여러 메시지가 짧은 시간에 연달아 들어와도 큐에서 순서대로 정확히 재생되도록 조치했습니다.
* 정상적으로 재생이 종료된 경우까지 발음 실패로 잘못 기록되던 문제를 수정했습니다.
* 숫자 읽는 방식이 더 자연스러워졌습니다.
  * "25"처럼 일반적인 숫자는 자릿수에 맞게 정확히 읽습니다.
  * "10시"처럼 시간을 나타낼 때는 고유어 숫자(열 시, 한 시 등)로 읽습니다.
  * 전화번호나 코드처럼 보이는 숫자는 한 글자씩 읽습니다.
* 관리자 페이지 에러 로그에 필터 기능이 추가되었습니다.
  * 연도, 월, 일을 지정해서 원하는 기간의 로그를 확인할 수 있습니다.
  * 로그 종류(ERROR/WARN)별로도 필터링할 수 있습니다.

</details>

<details>
<summary><strong>2026-07-31</strong> — Edge TTS 재생 딜레이 개선</summary>

* Edge 목소리 재생이 시작되기까지 걸리는 시간이 줄었습니다.
  * 음성을 전부 받아온 뒤 재생하던 방식에서, 도착하는 대로 바로 재생을 시작하는 방식으로 변경했습니다.
  * Edge tts로 재생 시 약 400ms -> 약 5ms 내외로 개선되었습니다.
* TTS 실패 시 알림이 더 정확해졌습니다.
  * 큐 모드에서도 생성이나 재생에 실패하면 실패 알림(이모지 반응)이 표시되도록 수정했습니다.
  * `/tts` 명령어 실행 중 오류가 발생하면 정확한 안내 메시지가 표시되도록 수정했습니다.

</details>

<details>
<summary><strong>2026-07-27</strong> — 로그인 보안 강화, 채팅 청소 기능 추가, 관리자 페이지 개선</summary>

* 로그인 보안이 강화되었습니다.
  * 로그인 시 위조된 요청(CSRF)을 막는 장치가 추가되었습니다.
  * 서버 응답에 보안 헤더(X-Frame-Options, Content-Security-Policy 등)가 추가되었습니다.
  * 서버 보안 설정(파일 접근 권한 등)을 점검하고 강화했습니다.
* TTS 읽기가 개선되었습니다.
  * 같은 글자를 6회 이상 반복 입력하면 5회까지만 읽도록 수정했습니다.
  * 이모지나 스티커와 텍스트를 함께 입력했을 때, 텍스트만 정확히 읽도록 수정되었습니다.
* 관리자가 채팅 내역을 조건에 맞게 정리할 수 있는 `/청소` 명령어가 추가되었습니다.
  * 범위, 기간, 작성자, 내용 조건을 선택해서 원하는 메시지만 삭제할 수 있습니다.
* 관리자 페이지 및 공지·문의 페이지가 수정되었습니다.
  * 관리자 페이지 새로고침 시 정상적으로 동작하지 않던 문제를 수정했습니다.
  * 배포 직후 화면이 깨지는 문제를 방지하는 조치를 적용했습니다.
  * 에러 로그 화면에서 항목을 클릭하면 바로 상세 내역이 펼쳐지도록 수정했습니다.
  * 공지사항과 문의 내용의 줄바꿈이 화면에 그대로 표시되도록 수정되었습니다.
  * 관리자 페이지에서 문의를 삭제할 수 있게 되었습니다.
  * 공지·문의 삭제 시 확인 창이 추가되었습니다.
