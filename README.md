...
* 서버 응답에 보안 헤더가 추가되어 여러 웹 공격 위험이 줄었습니다.
* 관리자 페이지 새로고침 시 정상적으로 동작하지 않던 문제를 수정했습니다.
* 배포 직후 화면이 깨지는 문제를 방지하는 조치를 적용했습니다.
* 에러 로그 화면에서 항목을 클릭하면 바로 상세 내역이 펼쳐지도록 더 간단해졌습니다.
* 서버 보안 설정(파일 접근 권한 등)을 점검하고 강화했습니다.
* 같은 글자를 너무 많이 반복해서 입력하면 일정 횟수까지만 읽도록 개선되었습니다.
...
* 공지·문의 삭제 시 확인 창이 뜨도록 개선되어 실수로 삭제되는 일이 줄었습니다.
